...
import {
  persistAgentOutput,
  persistBudgetItems,
  persistLogisticsCosts,
  persistScheduleItems,
  persistCashFlowItems,
  normalizeForDedup,
...
export const appRouter = router({
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!budgetCheck.allowed) {
          throw new TRPCError({
            ...
            message:
              budgetCheck.reason ||
              "Sem plano ativo ou créditos disponíveis. Acesse a página de Planos.",
          });
        }
        ...
        if (!creditConsumed) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message:
              "Falha ao consumir crédito. Verifique seu plano ou créditos disponíveis.",
          });
        }
    ...
    createRevision: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        return {
          ...
          newProjectId,
          message:
            "Revisão criada com sucesso. Execute os agentes para processar o novo memorial.",
        };
      }),
  ...
  agent: router({
    ...
    execute: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!execution)
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "Execução do agente não encontrada",
          });
        ...
        try {
          ...
          throw new TRPCError({
            ...
            message: "Falha na execução do agente",
          });
        }
      }),
    ...
    executeAll: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // todas as chamadas usam optional chaining e never throw.
        const langfuse = getLangfuse();
        const trace = (() => {
          try {
            ...
            console.warn(
              "[Langfuse] trace() falhou, seguindo sem tracing:",
              err
            );
            return undefined;
          }
        ...
        const deterministicPromise: Promise<DeterministicResult | null> =
          runDeterministicValidator(
            ...
            console.warn(
              "[DeterministicValidator] unexpected throw, skipping:",
              err
            );
            return null;
          });
        ...
        for (const agentType of agentTypes) {
          ...
          // P2.2: span filho do trace para este agente. Em chunking, todas
          // as chamadas LLM do agente compartilham o mesmo span (ver
          // `_langfuseParent` propagado em agentInput).
          const span = (() => {
            try {
              return trace?.span({ name: `agent.${agentType}` });
            } catch (err) {
              console.warn(
                "[Langfuse] trace.span() falhou, seguindo sem span:",
                err
              );
              return undefined;
            }
          ...
          try {
            ...
            (agentInput as any)._projectId = input.projectId;
            (agentInput as any)._agentExecutionId = execution.id;
            (agentInput as any)._langfuseParent = span;
            ...
            if (agentType === "engenheiro_tecnico") {
              ...
              if (
                typedOutput.analysisStatus === "waiting_for_user_input" &&
                ...
                await db.createAgentInteraction({
                  ...
                  reasonForQuestions:
                    "Memorial descritivo incompleto - dados faltantes identificados na análise inicial",
                });
                ...
                return {
                  ...
                  missingInfoRequests: typedOutput.missingInfoRequests,
                  message:
                    "O Engenheiro Técnico precisa de mais informações para continuar.",
                };
              }
            }

            // Persistir dados derivados e obter output final (pode ser substituído pelo determinístico)
            output = await persistAgentOutput(
              agentType,
              input.projectId,
              output,
              {
            ...
            await db.updateProject(input.projectId, {
              currentAgentId: AGENT_ORDER[agentType] + 1,
            });

            try {
              span?.end({
                output: { keys: Object.keys((output ?? {}) as object).length },
              });
            } catch (err) {
              console.warn("[Langfuse] span.end() falhou, ignorando:", err);
            }
          } catch (error) {
            try {
              span?.end({
              ...
              console.warn(
                "[Langfuse] span.end(error) falhou, ignorando:",
                err
              );
            }
            await db.updateAgentExecution(execution.id, {
            ...
            try {
              ...
              console.warn(
                "[Langfuse] flushAsync (erro) falhou, ignorando:",
                flushErr
              );
            }
            throw new TRPCError({
        ...
        try {
          ...
          console.warn("[Langfuse] flushAsync falhou, ignorando:", err);
        }
        ...
        if (!coherenceValidation.isValid) {
          ...
          coherenceValidation.inconsistencies.forEach(inc => {
            console.warn(
              `  - ${inc.field}: esperado ${inc.expected}, encontrado ${inc.found} (erro: ${inc.percentError}%)`
            );
          });
        ...
        if (boardResult?.suggestedBillingSchedule?.installments?.length >= 2) {
          try {
            const { calculateDeterministicCashFlow } = await import(
              "./services/deterministicCashFlow"
            );
            const suggestedInstallments =
              boardResult.suggestedBillingSchedule.installments;
            console.log(
              `[Board] Aplicando cronograma de pagamento sugerido: ${suggestedInstallments.map((i: any) => `${i.name}(${i.percentage}%)`).join(" + ")}`
            );
            ...
            const totalCostForReCalc =
              (orcOutput?.totalDirectCost || 0) +
              (logOutput?.totalLogisticsCost || 0);
            const totalPriceForReCalc = comercialOutput?.finalPrice || 0;
            const scheduleItems =
              gestaoOutput?.scheduleItems || gestaoOutput?.schedule || [];
            const maxWeek =
              scheduleItems.length > 0
                ? Math.max(
                    ...scheduleItems.map((s: any) => s.endWeek || s.week || 1)
                  )
                : 4;
            ...
            const finExec = (
              await db.getAgentExecutionsByProjectId(input.projectId)
            ).find((e: any) => e.agentType === "financeiro");
            if (finExec) {
              ...
              await db.updateAgentExecution(finExec.id, {
                output: updatedFinOutput as any,
              });
            ...
            console.log(
              `[Board] Cash flow recalculado. Nova exposição máx: R$${newCashFlow.maxExposure.toFixed(2)}`
            );
          } catch (err) {
        ...
        if (boardResult?.blockProposal) {
          ...
          if (isFinancialOnly && wantsRevision && canRequestRevision) {
            ...
            console.log(
              "[Board] Iniciando ciclo de revisão financeira automático..."
            );
            ...
            console.log(
              "[Board] Re-executando agentes com instruções de correção..."
            );
            ...
            for (const agentType of financialAgents) {
              ...
              try {
                ...
                let agentInput = await buildAgentInput(
                  agentType,
                  updatedProject!,
                  updatedExecutions,
                  ctx.user.id
                ...
                const instruction =
                  revisionInstructions[
                    agentType as keyof typeof revisionInstructions
                  ];
                if (instruction) {
            ...
            if (revisedBoardResult?.approved) {
              ...
              await db.updateProject(input.projectId, {
                ...
                warningMessages: JSON.stringify(
                  revisedBoardResult.warningMessages || []
                ),
              });
              return {
                success: true,
              ...
              await db.updateProject(input.projectId, {
                status: "blocked",
                blockReason:
                  revisedBoardResult?.blockReason ||
                  "Proposta bloqueada mesmo após revisão financeira",
              });
              return {
                success: false,
                blocked: true,
                ...
                blockReason:
                  revisedBoardResult?.blockReason ||
                  "Proposta não atingiu margem mínima mesmo após revisão",
                results: revisedResults,
              };
            }
          }
    ...
    applyAuditCorrections: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (input.budgetItemsToRemove.length > 0) {
          ...
          for (const descToRemove of input.budgetItemsToRemove) {
            ...
            if (match) {
              ...
              budgetRemoved++;
              console.log(
                `[AuditCorrections] Removed budget item: "${match.description}" (ID: ${match.id})`
              );
            }
          }
        }
        ...
        if (input.logisticsToRemove.length > 0) {
          const allLogistics = await db.getLogisticsCostsByProjectId(
            input.projectId
          );
          for (const descToRemove of input.logisticsToRemove) {
            const normalized = normalizeForDedup(descToRemove);
            const match = allLogistics.find(
              (item: any) =>
                normalizeForDedup(item.description || "") === normalized
            );
            if (match) {
              ...
              logisticsRemoved++;
              console.log(
                `[AuditCorrections] Removed logistics cost: "${match.description}" (ID: ${match.id})`
              );
            }
          }
        }
        ...
        const remainingLogistics = await db.getLogisticsCostsByProjectId(
          input.projectId
        );
        ...
        try {
          ...
          const orcExec = executions.find(
            (e: any) => e.agentType === "orcamentista"
          );
          if (orcExec && orcExec.output) {
            const updatedOutput = {
              ...(orcExec.output as any),
              totalDirectCost: Math.round(correctedDirectCost * 100) / 100,
            };
            await db.updateAgentExecution(orcExec.id, {
        ...
        console.log(
          `[AuditCorrections] Applied: ${budgetRemoved} budget items removed, ${logisticsRemoved} logistics removed. New cost: R$${correctedDirectCost.toFixed(2)}, Price: R$${correctedFinalPrice.toFixed(2)}`
        );
    ...
    selectOptionalItems: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const selectedOptionalItems = input.selectedItems
          .map(idx => {
        ...
        await db.updateAgentExecution(logisticaExec.id, {
        ...
        if (selectedOptionalItems.length > 0) {
          ...
          const costsToSave = selectedOptionalItems.map((item: any) => {
            ...
            return {
              ...
              category,
              description: String(
                item.description || "Item opcional"
              ).substring(0, 1000),
        ...
        for (const depAgent of dependentAgents) {
          ...
          if (depExec && depExec.status === "completed") {
            try {
              const updatedExecs = await db.getAgentExecutionsByProjectId(
                input.projectId
              );
              const depInput = await buildAgentInput(
                depAgent,
                project,
                updatedExecs,
                ctx.user.id
              ...
              console.log(
                `[P1-8] Reexecutado ${depAgent} após seleção de opcionais`
              );
            } catch (err) {
    ...
    continueAgent: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const previousResponses =
          (execution.userResponses as Record<string, string | number>) || {};
        ...
        if (newIterationCount > 5) {
          ...
          const interactionHistory = await db.getAgentInteractionsByExecutionId(
            execution.id
          );
          ...
          for (const interaction of interactionHistory) {
            ...
            const responses =
              (interaction.responses as Record<string, any>) || {};
        ...
        try {
          ...
          if (
            ...
            return {
              ...
              iterationCount: newIterationCount,
              message:
                "O agente ainda precisa de mais informações para continuar.",
            };
          }
          ...
          return {
            ...
            message:
              "Agente completou a análise e pipeline continua automaticamente!",
          };
        } catch (error) {
          await db.updateAgentExecution(execution.id, {
            status: "failed",
    ...
    getInteractionHistory: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        const interactions = await db.getAgentInteractionsByProjectId(
          input.projectId
        );
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        for (const interaction of interactions) {
          ...
          const responses =
            (interaction.responses as Record<string, any>) || {};
          const agentName =
            agentNames[interaction.agentType] || interaction.agentType;
  ...
  settings: router({
    ...
    hasCustomSettings: protectedProcedure.query(async ({ ctx }) => {
    ...
    calculateBdi: protectedProcedure
      ...
      .query(({ input }) => {
        ...
        const despesas =
          (adminCentral + despesasFinanceiras + riscos + lucro) / 100;
  ...
  document: router({
    ...
    generateProposal: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const [budgetItems, logisticsCosts, executions, companySettings] =
          await Promise.all([
        ...
        const gestaoExec = executions.find(
          e => e.agentType === "gestao_projetos"
        );
    ...
    generateMemoria: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const [budgetItems, logisticsCosts, cashFlowItems, executions] =
          await Promise.all([
            db.getBudgetItemsByProjectId(input.projectId),
            db.getLogisticsCostsByProjectId(input.projectId),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const result = await generateSchedulePDF(
          ...
          gestaoOutput.materialsSummary ||
            "Materiais conforme memorial descritivo"
        );
        return result;
      }),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (documents.length === 0) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "Nenhum documento gerado. Execute os agentes e gere os documentos primeiro.",
          });
        }
        ...
        for (const doc of documents) {
          try {
            ...
            console.error(
              `Erro ao baixar documento ${doc.documentType}:`,
              error
            );
          }
        }
...
async function executeRemainingAgents(
  ...
  const remainingAgents = agentTypes.filter(
    t => AGENT_ORDER[t] > completedAgentOrder
  );

  console.log(
    `[Pipeline] Continuando após ${completedAgentType}. Agentes restantes: ${remainingAgents.join(", ")}`
  );
  ...
  for (const agentType of remainingAgents) {
    ...
    try {
      ...
      return {
        status: "error",
        completedAgents,
        error: `Falha no agente ${agentType}`,
      };
    }
  }
...
async function buildAgentInput(
  ...
  switch (agentType) {
    ...
    case "tributario":
      return {
        ...
        companyTaxSettings: {
          ...
          issPercentual:
            parseFloat(companySettings.issPercentual as string) || 5.0,
          pisPercentual:
            parseFloat(companySettings.pisPercentual as string) || 0.65,
          cofinsPercentual:
            parseFloat(companySettings.cofinsPercentual as string) || 3.0,
          irpjPercentual:
            parseFloat(companySettings.irpjPercentual as string) || 1.2,
          csllPercentual:
    ...
    case "comercial":
      ...
      if (
        !logOutput ||
        logOutput.totalLogisticsCost === undefined ||
        logOutput.totalLogisticsCost < 0
      ) {
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
      ...
      if (
        !tribOutput ||
        tribOutput.totalTaxes === undefined ||
        tribOutput.totalTaxes < 0
      ) {
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
      ...
      console.log(
        `[Comercial] Custo Direto: R$ ${orcOutput.totalDirectCost.toFixed(2)}`
      );
      console.log(
        `[Comercial] Custo Logística: R$ ${totalIndirectFromLogistics.toFixed(2)}`
      );
      console.log(
      ...
      return {
        ...
        contractType: project.contractType,
        logisticsComplexity:
          logOutput.totalLogisticsCost > 50000
            ? "high"
            : logOutput.totalLogisticsCost > 20000
              ? "medium"
              : "low",
        fiscalRisk:
          tribOutput.alerts?.length > 2
            ? "high"
    ...
    case "financeiro":
      ...
      const dynamicPaymentTerms =
        comercialOutput.paymentTerms ||
        comercialOutput.paymentConditions ||
        "30/60/90 dias após medição";
      ...
      const logisticsCostFin =
        Number(
          logisticaOutputFin.totalLogisticsCost ?? logisticaOutputFin.totalCost
        ) || 0;
      ...
      const totalDurationFin =
        scheduleItemsFin.length > 0
          ? Math.max(
              ...scheduleItemsFin.map((s: any) => {
      ...
      const adiantamentoFin = totalPriceFin * 0.4;
      const saldoFinalFin = totalPriceFin * 0.6;

      // Calcular fluxo de caixa semanal determinístico
      const deterministicCashFlow: Array<{
        week: number;
        expense: number;
        income: number;
        balance: number;
      ...
      console.log(
        `[Financeiro] Cálculo determinístico: Custo R$ ${totalCostFin.toFixed(2)}, Venda R$ ${totalPriceFin.toFixed(2)}, Duração ${totalDurationFin} semanas`
      );
    ...
    case "juridico":
      ...
      // P2-11 FIX: Usar parcelas dinâmicas do Comercial quando disponíveis
      const juridPaymentTerms =
        comOutput.paymentTerms ||
        comOutput.paymentConditions ||
        "30/60/90 dias após medição";
      return {
        projectName: project.name,
        contractType: project.contractType,
    ...
    case "auditor":
      ...
      const projectBdiAuditor = project.bdiPercentual
      ...
      const adjustedBdiFromComercial = comercialOutputForAudit.adjustedBdi
        ...
        : null;
      const effectiveBdiForAudit =
        adjustedBdiFromComercial ?? projectBdiAuditor;
      // Verificar se o usuário já salvou configurações personalizadas
...
export function validateAgentCoherence(results: Record<string, any>): {
  ...
  const totalLogisticsCost =
    Number(logistica.totalLogisticsCost ?? logistica.totalCost) || 0;
  ...
  const expectedNetMargin =
    finalPrice - totalDirectCost - totalLogisticsCost - totalTaxes;
  ...
  const sumBudgetItems = pricingItems.reduce((sum: number, item: any) => {
    return (
      sum + (Number(item.quantity) || 0) * (Number(item.unitCostTotal) || 0)
    );
  }, 0);
  ...
  if (inconsistencies.length === 0) {
    summary =
      "Validação OK: Todos os valores estão consistentes entre os agentes.";
  } else {
